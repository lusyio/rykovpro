...
<?
	...
	$arFilter = Array("IBLOCK_ID"=> $arParams["IBLOCK_ID"], "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_BX_SERVICES_TOP_VALUE" => "Да");
...
<?while($ob = $res->GetNextElement()):?>
	...
	<?endif;?>
	<div class = "services-top" data-img = "<?=$picture;?>">
		<a href = "<?=$arFields['DETAIL_PAGE_URL'];?>"><?=$arFields["NAME"];?></a>
		<?if($arFields["DATE_ACTIVE_FROM"]):?>
			<?$date = substr($arFields["DATE_ACTIVE_FROM"], 0, 10);?>
			<span class = "services-top-date"><?=$date;?></span>
		<?endif;?>
		<span class = "services-top-bookmark">Топ услуга</span>
	</div>
	<?if(is_array($tags)):?>
		...
		<div class = "services-list">
			<div class = "row">
				...
				<div class = "col-12 col-sm-12 col-md-4">
					<div id = "<?=$this->GetEditAreaId($arItem['ID']);?>" class = "services-list-item">
						<div class = "services-list-item-picture">
							<img class = "img-fluid" src = "<?=$picture;?>" alt = "<?=$alt;?>">
						</div>
						<?if($arParams["DISPLAY_DATE"] != "N" && $arItem["DISPLAY_ACTIVE_FROM"]):?>
							<span class = "services-list-item-date"><?=$arItem["DISPLAY_ACTIVE_FROM"];?></span>
						<?endif;?>
						<?if($arItem["TAGS"]):?>
							<?$tags = explode(",", $arItem["TAGS"]);?>
							<div class = "services-list-item-tags">
								<ul>
									<?foreach($tags as $key => $value):?>
										<li><a href = "/search/?tags=<?=trim($value);?>">#<?=trim($value);?></a></li>
									<?endforeach;?>
						...
						<?endif;?>
						<a class = "services-list-item-link" href = "<?=$arItem['DETAIL_PAGE_URL'];?>"><?=$arItem["NAME"];?></a>
					</div>
				</div>
				<?endforeach;?>
			</div>
